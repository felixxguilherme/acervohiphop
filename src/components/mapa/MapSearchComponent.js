  ...
  return (
    ...
    >
      <div className="bg-black/90 backdrop-blur-md border-2 border-yellow-400 rounded-lg shadow-2xl">
        ...
        <div className="p-4 border-b border-theme/20">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 bg-transparent border border-theme/30 rounded text-theme placeholder-white/60 text-sm font-mono focus:border-yellow-400 focus:outline-none transition-colors duration-300"
            />
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-theme/60">
              🔍
            </div>
          </div>
              ...
              <div className="max-h-96 overflow-y-auto">
                        ...
                        <motion.button
                          ...
                          className={`w-full text-left p-3 rounded border transition-colors ${
                            selectedLocation?.id === location.id
                              ? 'bg-yellow-400/20 border-yellow-400 text-theme'
                              : 'bg-white/5 border-theme/20 hover:border-theme/40 text-theme/90'
                          }`}
                        >
                          <div className="flex items-start justify-between">
                            <div>
                              <p className="font-semibold text-sm mb-1">{location.name}</p>
                              <p className="text-xs text-theme/70 line-clamp-2">
                                {location.description}
                              </p>
                            </div>
                            <span className="bg-yellow-400/80 text-theme px-2 py-1 rounded text-xs font-bold ml-2 flex-shrink-0">
                              {location.itemCount}
                            </span>
                          </div>
                        </motion.button>
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-2">
                      {filteredItems.slice(0, 5).map((item) => {
                        ...
                        return (
                          <motion.button
                            ...
                            whileHover={{ x: 4 }}
                            className="w-full text-left p-3 rounded bg-white/5 border border-theme/20 hover:border-theme/40 transition-colors"
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="font-semibold text-sm text-theme mb-1 line-clamp-1">
                                  {item.title}
                                </p>
                                <p className="text-xs text-theme/70 line-clamp-2 mb-1">
                      ...
                      {filteredItems.length > 5 && (
                        <p className="text-theme/60 text-xs text-center py-2">
                ...
                {searchTerm && filteredLocations.length === 0 && filteredItems.length === 0 && (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-theme/80 text-sm font-semibold mb-1">
                      Nenhum resultado encontrado
                    </p>
                    <p className="text-theme/60 text-xs">
        ...
        {/* Quick Stats */}
        {!isExpanded && !searchTerm && (
          <div className="p-4 border-t border-theme/10">
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <p className="text-yellow-400 text-lg font-bold">
                  {atomMapResponse.locations.length}
                </p>
                <p className="text-theme/60 text-xs">Regiões</p>
              </div>
              <div>
                <p className="text-yellow-400 text-lg font-bold">
                  {atomMapResponse.locations.reduce((sum, loc) => sum + loc.itemCount, 0)}
                </p>
                <p className="text-theme/60 text-xs">Itens</p>
              </div>
            </div>
          </div>
        )}
